...
<div id="game-server-list" class="modal-body">
    <c:forEach items="${fns:getCurrentGameServerList()}" var="item" varStatus="serverIdStatus">
        ...
        <BUTTON id="${item.worldId}" textValue="${item.name}" status="${item.status}"  followerId="${item.followerId }" checkedStatus="0" onclick="isServerChecked(this)" class="btn <c:if test="${item.status == 0}">btn-success</c:if><c:if test="${item.status == 2}">btn-danger</c:if> ">${item.name}</BUTTON>
    </c:forEach>
        <%--<BUTTON  class="btn btn-success"><spring:message code='str1513'/></BUTTON>--%>
</div>
...
<script type="text/javascript">
    ...
    $(function(){
        selectedGameServerId = top.frames["mainFrame"].document.getElementById("serverIds").value;
        $("#game-server-list BUTTON").each(function(){
            var id = this.attributes.id.nodeValue;
            if(id == selectedGameServerId) {
    ...
    function myCallback(){
        ...
        top.frames["mainFrame"].document.getElementById("serverIds").value = selectedGameServerId;
    }
