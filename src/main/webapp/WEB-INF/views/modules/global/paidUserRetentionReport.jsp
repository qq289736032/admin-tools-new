...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/log/paidUserRetain/paidView"><spring:message code='str1690'/></a></li>
	</ul>
	<form id="searchForm" action="${ctx}/log/paidUserRetain/paidView" method="post" class="breadcrumb form-search">
		...
		<spring:message code='str1692'/><input type="text" name="startAmount" class="input_search" size="10" maxlength="20" value="${paramMap.startAmount}" onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"  id="startAmount">
		~&nbsp;<input type="text" name="endAmount"   class="input_search" size="10" value="${paramMap.endAmount}"  onkeyup="if(this.value.length==1){this.value=this.value.replace(/[^1-9]/g,'')}else{this.value=this.value.replace(/\D/g,'')}"   id="endAmount">
		...
		<input type="button" class="btn btn-primary" value="跳转（付费用户再次充值）" onclick="window.location.href='${ctx}/log/paidDay/paidView'">
		&nbsp;<input id="btnSubmit" class="btn btn-primary" type="submit" value="<spring:message code="operation.search"/>" onclick="return page();"/>
	</form>
	<tags:message content="${message}"/>
	<div class="panel panel-primary">
	<div class="panel-heading"><spring:message code='str1690'/></div>
