...
public class Page<T> {
    ...
    private int configPageSize = Integer.valueOf(Global.getConfig("page.pageSize"));
    ...
    private  String  xiegye="";
    private  String  daqian="";
    private String tiao="";
    ...
    public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize) {
    	...
    	WebApplicationContext ac = RequestContextUtils.getWebApplicationContext(request);
    	 shangye=ac.getMessage("str1666", null,"上一页", locale);
    	 xiegye=ac.getMessage("str1667", null,"下一页", locale);
    	 daqian=ac.getMessage("str1668", null,"当前", locale);
    	 tiao=ac.getMessage("str1669", null,"条", locale);
    	 total=ac.getMessage("str1670", null,"共", locale);
    ...
    public String toString() {
        ...
        if (pageNo == first) {// 如果是首页
            sb.append("<li class=\"disabled\"><a href=\"javascript:\">&#171;"+shangye+"</a></li>\n");
        } else {
            sb.append("<li><a href=\"javascript:\" onclick=\"" + funcName + "(" + prev + "," + pageSize + ");\">&#171;"+shangye+"</a></li>\n");
        }
        ...
        if (pageNo == last) {
            sb.append("<li class=\"disabled\"><a href=\"javascript:\">"+xiegye+" &#187;</a></li>\n");
        } else {
            sb.append("<li><a href=\"javascript:\" onclick=\"" + funcName + "(" + next + "," + pageSize + ");\">"
                    +xiegye+ " &#187;</a></li>\n");
        }
        int[] pageSizeArr = new int[]{configPageSize, 30, 60, 100};
        sb.append("<li class=\"disabled\"><select name=\"selectSize\" style=\"width:60px\" onchange=\""+funcName+"(1, this.value);\">");
        for (int size : pageSizeArr) {
        	if (size == pageSize) {
        		sb.append("<option selected=\"selected\" value=\"" + size + "\">" + size + "</option>");
        	} else {
        		sb.append("<option value=\"" + size + "\">" + size + "</option>");
        	}
        }
        sb.append("</select></li>\n");
        sb.append("<li class=\"disabled controls\"><a href=\"javascript:\"> "+daqian);
        ...
        sb.append(funcName + "(" + pageNo + ",this.value);\" onclick=\"this.select();\"/> "+tiao+"，");
        sb.append(total + count +tiao + (message != null ? message : "") + "</a><li>\n");
