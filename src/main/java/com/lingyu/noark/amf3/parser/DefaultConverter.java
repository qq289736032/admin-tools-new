...
import com.lingyu.noark.amf3.util.ClassUtil;
...
public class DefaultConverter implements Converter {
	...
	public Object convertForDeserialization(Object value, Type targetType) {
		...
		if (value == null) {
			...
			} // Should only be an Integer or Double (after AMF3
				// deserialization).
			...
			} // Java Character -[AMF3]-> AS3 String[1] -[AMF3]-> java
				// Character.
	...
	protected boolean canConvertForMethodInvocation(Object value, Type targetType) {
		...
		if (targetClass.isAssignableFrom(valueClass)) {
			return true; // Class/Primitive Identity & Widening Primitive
							// Conversion.
		}
		if (targetClass.isPrimitive()) {
			if ((valueClass == Integer.class && (targetClass == Integer.TYPE || targetClass == Long.TYPE || targetClass == Float.TYPE || targetClass == Double.TYPE))
			...
			if ((valueClass == Integer.class && (targetClass == Long.class || targetClass == Float.class || targetClass == Double.class
					...
					|| (valueClass == Long.class && (targetClass == Float.class || targetClass == Double.class || targetClass == BigDecimal.class || targetClass == BigInteger.class))
					|| (valueClass == Float.class && (targetClass == Double.class || targetClass == BigDecimal.class || targetClass == BigInteger.class))
					|| (valueClass == Byte.class && (targetClass == Short.class || targetClass == Integer.class || targetClass == Long.class
							|| targetClass == Float.class || targetClass == Double.class || targetClass == BigDecimal.class || targetClass == BigInteger.class))
					|| (valueClass == Short.class && (targetClass == Integer.class || targetClass == Long.class || targetClass == Float.class
							|| targetClass == Double.class || targetClass == BigDecimal.class || targetClass == BigInteger.class))
					|| (valueClass == Character.class && (targetClass == Integer.class || targetClass == Long.class || targetClass == Float.class
							|| targetClass == Double.class || targetClass == BigDecimal.class || targetClass == BigInteger.class))) {
				return true;
			}
			...
			if (!isExternalizable() && !Map.class.isAssignableFrom(type) && !Hashtable.class.isAssignableFrom(type)) {
				try {
					...
					for (Field field : fields) {
						...
						if (!propertyNames.contains(propertyName) && !Modifier.isStatic(field.getModifiers()) && !Modifier.isTransient(field.getModifiers())) {
